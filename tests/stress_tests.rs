...
fn check_string_field(v: &serde_json::Value, path: &[&str], check_non_empty: bool) {
    ...
    if check_non_empty {
        assert!(
            !current.as_str().expect("字符串字段应有值").is_empty(),
...
fn test_st_json_001_top_level_fields() {
    ...
    for key in v.as_object().expect("顶层JSON应为对象").keys() {
        assert!(
...
fn test_st_as_002_nonexistent_model() {
    ...
    let v: serde_json::Value = serde_json::from_str(&stdout).expect("JSON输出应该合法");
    // request 应包含模型名
    let request = v.get("request").expect("评估结果应含 request");
...
fn test_st_int_002_structure_consistent() {
    ...
    let first_system_keys: Vec<String> = {
        let v: serde_json::Value = serde_json::from_str(lines[0]).expect("JSON输出应该合法");
        let sys = v["system"].as_object().expect("system 应为对象");
