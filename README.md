...
## v0.6.0 — 精准打击 (2026-05-29)

### 🎯 缓存差距分析
你的缓存命中率 97%，但目标是 99%。差的 2% 去哪了？

```bash
hawk-eye-mem --analyze-cache-gaps
```
...
═══════════════════════════════════════
  ...
  📊 缺口分类:
    🟢 new_session_cold_start (50%) — 新会话冷启动
    🟢 model_switch (30%) — 模型切换重置前缀
    🟢 other (20%) — API重试等

  💡 修复建议:
    1. [LOW] 温度设0.0，减少上下文压缩
═══════════════════════════════════════
```

支持参数：
- `--days N`：分析天数（默认7天）
- `--target 99`：目标命中率（默认99%）
- `--json`：JSON格式输出
...
### 📈 测试覆盖
- 35个测试全绿（28原有+7新增）
- V0.6新增：心跳JSON验证、差距分析JSON/人类可读、参数生效验证
